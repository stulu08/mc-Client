...
public class CosmeticController {
    ...
    public static void checkUser(AbstractClientPlayer player) {
            for (int i = 0; i != Client.ClientUsers.length; i++) {
                if (player.getGameProfile().getId().equals(UUID.fromString(Client.ClientUsers[i]))) {
                    render = true;
                    return;
                }
            }
    ...
    //wither effect
    public static boolean shouldRenderWitherEffect(AbstractClientPlayer player){
        if(Client.EnableDragonWings) {
            if(!checked){
                checkUser(player);
            }
            return Client.Wither;
        }
        else{
            return false;
        }
